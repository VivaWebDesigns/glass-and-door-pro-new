...
function setupMockDb() {
  ...
  const values = vi.fn(
    (_record: { key: string; value: string; category: string; isSecret: boolean }) => ({
      returning,
    }),
  );
...
describe("SettingsStorage encryption configuration", () => {
  afterEach(() => vi.unstubAllEnvs());
  it("rejects secret writes before database access when no encryption key exists", async () => {
    vi.clearAllMocks();
    vi.stubEnv("SESSION_SECRET", "");
    await expect(
      new SettingsStorage().upsertSetting("token", "private-value", "email", true),
    ).rejects.toThrow("SESSION_SECRET is required");
    expect(mockDb.select).not.toHaveBeenCalled();
    expect(mockDb.insert).not.toHaveBeenCalled();
  });
  it("round trips encrypted settings with an explicitly configured key", async () => {
    vi.stubEnv("SESSION_SECRET", "unit-test-only-settings-key");
    const { where, returning, values } = setupMockDb();
    where.mockResolvedValue([]);
    returning.mockResolvedValue([{ key: "token", category: "email", isSecret: true }]);
    const storage = new SettingsStorage();
    await storage.upsertSetting("token", "private-value", "email", true);
    const stored = values.mock.calls[0][0];
    expect(stored.value).not.toContain("private-value");
    where.mockResolvedValue([stored]);
    await expect(storage.getSetting("token")).resolves.toBe("private-value");
  });
});
