    ...
    >
      ...
      <div className="flex-shrink-0">
        {entry.excluded ? (
          <Badge
            variant="secondary"
            className="text-xs bg-slate-100 text-slate-500 dark:bg-slate-800"
          >
            Excluded
          </Badge>
        ) : (
...
export function CmsSitemapTab() {
  ...
  const handleCopyRobotsUrl = async () => {
    const origin = siteUrl || window.location.origin;
    const robotsUrl = `${origin.replace(/\/$/, "")}/robots.txt`;

    try {
      await navigator.clipboard.writeText(robotsUrl);
      toast({ title: "robots.txt URL copied" });
    } catch {
      toast({ title: "Could not copy robots.txt URL", variant: "destructive" });
    }
  };
