      ...
      <div className={`relative z-10 px-8 py-16 ${isSplit ? "max-w-2xl" : "max-w-3xl mx-auto"}`}>
        ...
        <h1 className={`font-heading font-bold text-white mb-4 leading-tight ${variant === "glass-home" ? "text-4xl md:text-6xl" : "text-4xl md:text-5xl"}`} style={headingTextStyle}>
          ...
          {accentHeading && (
            <>
              {" "}
              <span className="text-accent" style={accentHeadingTextStyle}>{renderPublicDisplayText(accentHeading)}</span>
            </>
          )}
        </h1>
...
function TestimonialsBlock({ props }: { props: Record<string, unknown> }) {
  ...
  const renderCard = (item: { quote: string; name: string; role: string; location: string; rating?: number; source?: string; sourceIcon?: string; date?: string }, i: number) => (
        ...
        <div className={variant === "gallery-strip" ? "grid grid-cols-2 gap-3 md:grid-cols-4" : `grid grid-cols-1 ${colsClass} ${gapClass}`}>
          {images.map((img, i) => (
            ...
            >
              ...
              />
              {img.caption && <p className={isProjectGallery ? "px-3 py-3 text-center text-sm font-medium text-slate-700" : "text-xs text-muted-foreground text-center mt-1"}>{img.caption}</p>}
            </div>
          ))}
        </div>
      )}
